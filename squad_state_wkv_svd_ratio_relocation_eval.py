...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate RWKV with relocation + ratio-threshold SVD-compressed WKV state."
    )
...
def relocate_matrix_by_value(matrix: torch.Tensor):
    flat = matrix.reshape(-1)
    order = torch.argsort(flat.float(), descending=True)
    relocated_flat = flat.index_select(0, order)
    relocated = relocated_flat.reshape_as(matrix)
    return relocated, order
...
def restore_matrix_from_relocation(relocated_matrix: torch.Tensor, order: torch.Tensor):
    ...
    restored_flat.scatter_(0, order, relocated_flat)
    return restored_flat.reshape_as(relocated_matrix)


def apply_relocation_svd_ratio_to_wkv_state(state, threshold: float):
    ...
    for i in range(0, len(out), 3):
        ...
        for h in range(num_heads):
            relocated, order = relocate_matrix_by_value(wkv[h])
...
def init_threshold_record_file(output_dir: str, threshold: float) -> str:
    ...
    path = os.path.join(output_dir, f"relocation_svd_threshold_{name}_qa_records.jsonl")
...
def plot_ratio_relocation_results(summary: OrderedDict, output_path: str):
    keys = list(summary.keys())
    accs = [summary[k]["accuracy"] for k in keys]
    mean_compressions = [summary[k]["mean_compression_ratio"] for k in keys]
    x = list(range(len(keys)))
    plt.figure(figsize=(8, 6))
    bars = plt.bar(x, accs, color="#55A868", edgecolor="white", linewidth=0.8, zorder=5)
    ...
    plt.title("Relocation + Ratio-threshold SVD Compression", pad=12)
    plt.xticks(x, keys)
...
def evaluate_with_threshold(
    ...
    ratio_sum = 0.0
    ratio_count = 0
    progress = tqdm(range(len(dataset)), desc=f"Relocation+SVD Eval (x={threshold:.2f})")
    for idx in progress:
        row = dataset[idx]
        svd_state, rs, rc = apply_relocation_svd_ratio_to_wkv_state(row["state"], threshold)
        ratio_sum += rs
        ratio_count += rc
...
def main():
    ...
    result_json = os.path.join(output_dir, "squad_rwkv_wkv_svd_ratio_relocation_eval_results.json")
    with open(result_json, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    plot_path = os.path.join(output_dir, "wkv_svd_ratio_relocation_bar.png")
    plot_ratio_relocation_results(results, plot_path)

    print(f"Saved summary: {result_json}")
    print(f"Saved plot: {plot_path}")
    for threshold, row in results.items():
        print(
            f"Relocation SV threshold {threshold}: "
